...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title={
        <div className="flex items-center gap-2 text-ink-900 font-bold text-base">
      ...
      footer={
        <div className="flex justify-end gap-2">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-ink-900 hover:bg-ink-800 active:scale-[0.98] text-white rounded-lg text-xs font-semibold shadow-xs transition-all"
          >
            {t('close', 'Close')}
          </button>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 bg-indigo-50 text-indigo-700 rounded border border-indigo-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
            ...
            <button
              ...
              className={`p-3.5 rounded-lg border text-left transition-all flex flex-col justify-between ${
                language === 'en'
                  ...
                  : 'border-rule bg-white hover:border-paper-400 hover:bg-paper-100/50'
              }`}
            >
              <div className="flex items-center justify-between w-full mb-1">
                <span className="text-[13px] font-bold text-ink-900">English</span>
                {language === 'en' && (
                  <span className="w-5 h-5 rounded-full bg-indigo-700 text-white flex items-center justify-center shrink-0">
            ...
            >
              <div className="flex items-center justify-between w-full mb-1 flex-row-reverse">
                <span className="text-[13px] font-bold text-ink-900 font-['Cairo']">العربية</span>
                {language === 'ar' && (
                  <span className="w-5 h-5 rounded-full bg-indigo-700 text-white flex items-center justify-center shrink-0">
                    <Check className="w-3 h-3 stroke-[3]" />
                  </span>
              ...
              <span className="text-[11px] text-ink-soft font-['Cairo']">
        ...
        {currentUser && (
          <div className="border-t border-ink-100 pt-4 space-y-3">
            ...
            <div className="bg-paper-100/70 border border-rule rounded-lg p-3.5 space-y-2.5 text-xs">
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between border-t border-rule/50 pt-2">
                ...
                <span className="font-mono font-medium text-ink-800 bg-white px-2 py-0.5 rounded border border-rule">
                  {currentUser.role}
                </span>
              </div>
