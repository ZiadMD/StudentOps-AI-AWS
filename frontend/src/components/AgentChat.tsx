    ...
    <div className="flex flex-col h-full bg-[#F8FAFC]">
        ...
        <div className="max-w-3xl mx-auto space-y-8">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center mt-20 text-center space-y-4 opacity-90">
              ...
              <h2 className="text-xl font-medium text-ink-900 tracking-tight">How can I help you today?</h2>
          ...
          {messages.map((msg, i) => {
            ...
            return (
              ...
              >
                {msg.role === 'user' ? (
                  <div className="max-w-[80%] bg-paper-200/80 px-4 py-3 rounded-2xl text-ink-900 text-[15px] leading-relaxed shadow-sm">
                    {msg.content}
                  </div>
                ) : (
                        ...
                        <Card className="mt-4 border-green-200 bg-green-50/30">
                          <div className="p-4 flex items-start space-x-3">
                            <div className="space-y-3 flex-1">
                              <div>
                                <h4 className="text-sm font-semibold text-ink-900">Authorization Required</h4>
                                <p className="text-xs text-ink-soft mt-0.5">Review and confirm before dispatching.</p>
                              </div>
                              <pre className="p-3 bg-white border border-rule rounded-lg font-mono text-[11px] text-ink-800 whitespace-pre-wrap overflow-x-auto">
      ...
      >
        <div className="max-w-3xl mx-auto">
          <div className="relative bg-white border border-rule shadow-[0_2px_10px_rgba(0,0,0,0.04)] rounded-2xl overflow-hidden focus-within:border-paper-400 focus-within:shadow-[0_4px_16px_rgba(0,0,0,0.08)] transition-all flex flex-col">
            <textarea
              ref={textareaRef}
              value={input}
              ...
              className="w-full max-h-48 min-h-[56px] resize-none bg-transparent py-4 px-4 pr-12 text-[15px] outline-none text-ink-900 placeholder-ink-400 disabled:opacity-60"
              rows={1}
            />
            <div className="flex items-center justify-between px-3 pb-3 pt-1">
              <button className="p-2 text-ink-faint hover:text-ink-soft transition-colors rounded-lg hover:bg-paper-200">
              ...
              <button
                ...
                className={`p-2 rounded-lg transition-all flex items-center justify-center ${
                  input.trim() && !loading
                    ? 'bg-black text-white hover:bg-ink-800 active:scale-[0.98]'
                    : 'bg-paper-200 text-ink-faint cursor-not-allowed'
                }`}
              >
...
const ToolAccordion: React.FC<{ trace: ToolCallExecution }> = ({ trace }) => {
  ...
  return (
    <div className="border border-rule/60 bg-paper-100/50 rounded-lg overflow-hidden text-sm w-fit min-w-[280px]">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center justify-between w-full p-2.5 hover:bg-paper-200/50 transition-colors text-ink-soft"
      >
        <div className="flex items-center space-x-2">
          <Code className="w-4 h-4 text-ink-faint" />
          <span className="font-medium text-xs">
            Used tool:{' '}
            <span className="font-mono text-[11px] text-ink-800 bg-white px-1 py-0.5 rounded border border-rule ml-1">
              {trace.tool_name}
            </span>
          </span>
        </div>
      ...
      {open && (
        <div className="p-3 border-t border-rule/60 bg-white space-y-3">
          {trace.reasoning_summary && (
            <p className="text-[11px] text-ink-soft italic">{trace.reasoning_summary}</p>
          )}
          <div>
            <span className="text-[10px] font-bold uppercase tracking-wider text-ink-faint">Result</span>
            <pre className="mt-1 p-2 bg-paper-100 rounded border border-ink-100 font-mono text-[10px] text-ink-soft whitespace-pre-wrap max-h-40 overflow-y-auto">
