...
export const RegisterPage: React.FC<RegisterPageProps> = ({ onRegister, onGoToLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-ambient-mesh flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-md">
          ...
          {[1, 2].map((s) => (
            <React.Fragment key={s}>
              <div className={`flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold transition-colors ${
                step > s ? 'bg-emerald-500 text-white' : step === s ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-500'
              }`}>
        ...
        {step === 1 && (
          <form onSubmit={handleStep1} className="space-y-6">
            <div>
              <h1 className="text-[28px] font-bold text-slate-900 tracking-tight">Create an account</h1>
            ...
            <button
              type="submit"
              className="w-full py-2.5 px-4 bg-blue-600 hover:bg-blue-700 active:scale-[0.99] text-white font-semibold rounded-lg text-sm transition-all shadow-sm flex items-center justify-center space-x-2"
            >
        ...
        {step === 2 && (
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <h1 className="text-[28px] font-bold text-slate-900 tracking-tight">Your profile</h1>
              <p className="text-sm text-slate-500 mt-1.5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-3 py-3 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all shadow-xs"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-3 py-3 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all shadow-xs font-cairo"
            ...
            {teams.length > 0 && (
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full px-3 py-3 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all shadow-xs"
                >
                  {teams.map(t => (
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 px-4 bg-blue-600 hover:bg-blue-700 active:scale-[0.99] text-white font-semibold rounded-lg text-sm transition-all shadow-sm flex items-center justify-center space-x-2 disabled:opacity-60"
            >
              {loading ? (
