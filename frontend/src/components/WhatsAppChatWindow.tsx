...
export const WhatsAppChatWindow: React.FC<WhatsAppChatWindowProps> = ({ currentUser }) => {
  ...
  return (
    <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm overflow-hidden flex flex-col h-[750px] max-h-[85vh]">
      {/* Top Bar / Header */}
      <div className="bg-slate-900 text-white px-4 sm:px-5 py-3 flex items-center justify-between gap-3 border-b border-slate-800">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse shrink-0" />
          <h2 className="text-sm font-semibold tracking-tight text-white truncate">
            WhatsApp Operational Chat
          </h2>
          <span className="hidden md:inline text-[11px] bg-slate-800 text-slate-300 px-2 py-0.5 rounded-full border border-slate-700 whitespace-nowrap">
            OpenWA Official SIM
          </span>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={`flex-1 min-w-0 flex flex-col bg-slate-50 ${activeStudentId ? 'flex' : 'hidden sm:flex'}`}>
          {activeThread ? (
            <>
